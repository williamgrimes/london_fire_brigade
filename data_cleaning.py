"""functions for cleaning lfb data"""
...
def convert_date_time(df):
    ...
    df['DDDateTimeOfCall'] = df['DDDateTimeOfCall'].apply(str)
    df['DDDateTimeOfCall'] = df["DDDateTimeOfCall"].apply(lambda x: datetime.strptime(x,"%Y-%m-%d %H:%M:%S"))
    return df
